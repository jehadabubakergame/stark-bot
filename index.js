...
// ==================== إحصائيات الرسائل والصوت ====================
const STATS_FILE = './stats.json';
const INFO_CHANNEL_ID = '1480695622582407419';
...
function buildStatsEmbed(guildId) {
    ...
    for (const [userId, data] of Object.entries(guildStats)) {
        ...
        voiceRanking.push({ userId, voiceMs });
        messageRanking.push({ userId, messages: data.messages || 0 });
    }

    voiceRanking.sort((a, b) => b.voiceMs - a.voiceMs);
    ...
    let voiceText = topVoice.length
        ? topVoice.map((u, i) => `#${i + 1} <@${u.userId}> — ${formatTime(u.voiceMs)}`).join('\n')
        : 'لا يوجد بيانات';

    let messageText = topMessages.length
        ? topMessages.map((u, i) => `#${i + 1} <@${u.userId}> — ${u.messages} رسالة`).join('\n')
        : 'لا يوجد بيانات';
    ...
    return new EmbedBuilder()
        ...
        .setTitle('🏆 إحصائيات السيرفر')
        .addFields(
            { name: '🎙️ Top 10 Voice', value: voiceText, inline: false },
            { name: '💬 Top 10 Messages', value: messageText, inline: false }
        )
        .setTimestamp();
}

// يحسب الرسائل + أمر معلومات
client.on('messageCreate', message => {
    if (!message.guild || message.author.bot) return;
    ...
    if (message.channel.id !== INFO_CHANNEL_ID) return;
    if (message.content !== 'معلومات') return;

    const embed = buildStatsEmbed(message.guild.id);
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'stats') {
        const embed = buildStatsEmbed(interaction.guild.id);
        interaction.reply({ embeds: [embed] });
    }
});
